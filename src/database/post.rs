...
impl Post {
    ...
    pub async fn get(id: &str, pg: &PgPool) -> Result<Post> {
        let post = sqlx::query_as!(
            Post,
            r#"
            SELECT id, user_id, content, files, created_at 
            FROM app_post 
            WHERE app_post.id = $1;
            "#,
            id
    ...
    pub async fn get_all(username: &str, pg: &PgPool) -> Result<Vec<Post>> {
        let posts = sqlx::query_as!(
            Post,
            r#"
            SELECT app_post.id, app_user.id AS user_id, content, files, app_post.created_at 
            FROM app_post 
